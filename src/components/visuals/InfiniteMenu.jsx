...
export default function InfiniteMenu ({ items = [] }) {
  ...
  const handleButtonClickGit = () => {
    if (!activeItem?.linkGit) return
    if (activeItem.linkGit.startsWith('http')) {
      window.open(activeItem.linkGit, '_blank')
    } else {
      console.log('Internal route:', activeItem.linkGit)
    }
  }
        ...
        `}
          >
            <p className="select-none relative">
              <img src={assets.icon_github} alt="arrow-right"/>
            </p>
          </div>
